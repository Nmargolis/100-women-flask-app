...
def get_names():
    ...
    for speakerID in speaker_dict:
        names[speakerID] = speaker_dict[speakerID]["name"] # e.g. {0:"Erin"}
    ...
    if request.method == 'POST':
        ...
        if file and allowed_file(file.filename):
            ...
            for speaker_id, speaker in speaker_dict.items():
                ...
                #speaker["name"] = nlp.get_name_from_first_sentence(sentences) #or speaker_id
                speaker["name"] = str(nlp.get_name_from_first_sentence(sentences))  # or speaker_id
